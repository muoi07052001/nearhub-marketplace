...
impl MarketContract {
    ...
    pub fn get_supply_by_owner_id(&self, account_id: AccountId) -> U128 {
        let sales_by_owner_id = self.tokens_by_owner_id.get(&account_id);
    ...
    pub fn get_supply_by_contract_id(&self, contract_id: NFTContractId) -> U128 {
        let tokens_by_contract_id = self.tokens_by_contract_id.get(&contract_id);
    ...
    pub fn get_sales_by_owner_id(
        ...
        let tokens_by_owner_id = self.tokens_by_owner_id.get(&account_id);
        let contract_token_ids = if let Some(tokens_by_owner_id) = tokens_by_owner_id {
            tokens_by_owner_id
        } else {
            return vec![];
        };
